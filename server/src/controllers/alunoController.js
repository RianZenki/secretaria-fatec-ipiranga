...
function consultarDados(req, res) {
  ...
  db.query(`SELECT nome, email, curso, turno, ra FROM aluno WHERE idAluno =  '${id}'`,
    (err, result) => {
      if (err)
...
function alterarAluno(req, res) {
  ...
  db.query(`SELECT * FROM aluno WHERE idAluno =  '${idAluno}'`,
    (err, result) => {
      if (err)
      ...
      if (result.length > 0) {
        db.query(`
          ...
          return res.status(200).json({ msg: "Dados alterado com sucesso" })
...
function deletarAluno(req, res) {
  ...
  db.query(`SELECT * FROM aluno WHERE idAluno =  '${id}'`,
    (err, result) => {
      if (err)
...
module.exports = { consultarDados, alterarAluno, deletarAluno }