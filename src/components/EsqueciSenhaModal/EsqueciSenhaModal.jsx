...
export const EsqueciSenhaModal = (props) => {
  ...
  const validationEsqueciSenha = yup.object().shape({
    emailRec: yup.string().email("* Digite um email válido").required("* Campo obrigatório"),
  })

  const handleEsqueciSenha = async ({ emailRec }) => {
    setLoading(true)
    api.post('/auth/esqueci-senha', { emailRec })
      .then((response) => {
        if (response.status === 400) {
          setResposta({...response, tipo: "erro", texto: response.data.error})
          setMostrandoAlert(true)
          setTimeout(() => {setMostrandoAlert(false)}, 3000)
        }
  ...
  return (
    <>
      {mostrandoAlert && resposta && <Alert tipo={resposta.tipo} texto={resposta.texto} />}

      <div className={styles["fundo-container"]} onClick={() => props.setAberto(false)} />
      <div className={styles["modal-container"]}>
        <div className={styles["header"]}>
          <h1>Recuperar senha</h1>
          <p>Informe o e-mail cadastrado e será enviado o link para a criação de uma nova senha.</p>
        </div>
        ...
        >
          <Form className={styles["form"]}>
            <TextInput titulo="Endereço de e-mail" nome="emailRec" placeholder="aluno@email.com" tipo="email">
